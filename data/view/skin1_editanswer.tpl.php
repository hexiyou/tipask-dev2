...
<? } } ?>
 &gt; 
        <a href="<?=SITE_URL?>q-<?=$answer['qid']?>.html"><?=$question['title']?></a>&gt; 编辑回答
    </div>
     <div class="tie-left">

        <div class="clr"></div>
        <div class="leftc">
            <form name="questionform" onsubmit="return check_form();" action="<?=SITE_URL?>question/editanswer.html" method="post" >
                <input type="hidden" value="<?=$aid?>" name="aid" />
                <div class="tiwen">
                    <script type="text/plain" id="mycontent" name="content" style="width:720px;padding-left:10px;"><?=$answer['content']?></script>

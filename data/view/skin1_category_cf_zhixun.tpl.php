...
<? } } ?>
    ...
    <div class="list"> <span class="limenu"> 
      <? if(all==$status) { ?><a class="now">全部问题</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/all/2.html.html">全部问题</a><? } ?>      </h3>
      <? if(4==$status) { ?><a class="now">悬赏处方</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/4/2.html.html">悬赏处方</a><? } ?> 
      <? if(1==$status) { ?><a class="now"><font color="#ff6600">？</font>待解决</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/1/2.html.html"><font color="#ff6600">？</font>待解决</a><? } ?> 
      <? if(2==$status) { ?><a class="now"><font color="#1bbf00">√ </font>已解决</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/2/2.html.html"><font color="#1bbf00">√ </font>已解决</a><? } ?> 
      </span> 
      <!--冠名广告--> 
...
<? if(is_array($questionlist)) { foreach($questionlist as $question) { ?>
        <li class=" " onmouseover="this.className='bg9fc'" onmouseout="this.className=' '"> <span class="tit"> <a href="<?=$question['url']?>" class="bt" title="<?=$question['title']?>" target="_blank"><?=$question['title']?></a> [<a href="<?=SITE_URL?>c-<?=$question['cid']?>.html/all/2.html" target="_blank" class="ks"><?=$question['category_name']?></a>] </span> <span class="huifu"><?=$question['answers']?>/6</span> <span class="zhuan"><img src="<?=SITE_URL?>css/common/icn_<?=$question['status']?>.gif" /></span> <span class="zuiho"><?=$question['format_time']?></span> </li>

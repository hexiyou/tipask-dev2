...
<? if(is_array($sublist)) { foreach($sublist as $index => $sub) { ?>
                            <li> 
                                ...
                                <? } else { ?> 
                                <a href="index.php?c-<?=$sub['id']?>/all/<?=$ask_area?>.html"><? echo cutstr($sub['name'],10,''); ?></a>
                                <? } ?> 
                            </li>
...
<? } } ?>
                ...
                <div class="list"> 
                    <span class="limenu" style="display:none;"> 
                        <? if(all==$status) { ?><a class="now">全部问题</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/all/3.html.html">全部问题</a><? } ?> </h3>
                        <? if(4==$status) { ?><a class="now">秘方验方</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/4/3.html.html">秘方验方</a><? } ?> 
                        <? if(1==$status) { ?><a class="now"><font color="#ff6600">？</font>待解决</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/1/3.html.html"><font color="#ff6600">？</font>待解决</a><? } ?> 
                        <? if(2==$status) { ?><a class="now"><font color="#1bbf00">√ </font>已解决</a><? } else { ?><a href="<?=SITE_URL?>c-<?=$cid?>/2/3.html.html"><font color="#1bbf00">√ </font>已解决</a><? } ?> 
                    </span> 
                    <!--冠名广告--> 

                    <span class="lititle"><span>回复/点击</span><span>状态</span><span>最后回复</span></span>
                    <ul class="color06b">
                        
<? if(is_array($questionlist)) { foreach($questionlist as $question) { ?>
                        <li class=" " onmouseover="this.className='bg9fc'" onmouseout="this.className=' '"> <span class="tit"> <a href="<?=$question['url']?>" class="bt" title="<?=$question['title']?>" target="_blank"><?=$question['title']?></a> [<a href="<?=SITE_URL?>c-<?=$question['cid']?>.html/all/3.html" target="_blank" class="ks"><?=$question['category_name']?></a>] </span> <span class="huifu"><?=$question['answers']?>/6</span> <span class="zhuan"><img src="<?=SITE_URL?>css/common/icn_<?=$question['status']?>.gif" /></span> <span class="zuiho"><?=$question['format_time']?></span> </li>

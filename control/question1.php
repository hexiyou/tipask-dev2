...
class questioncontrol extends base {
    ...
    function onadd() {
        ...
        if (isset($this->post['submit'])) {
            ...
            $cid = $this->post['cid'];
            $tags = trim($this->post['qtags']);
            @$hidanswer = $this->post['hidanswer'] ? 1 : 0;
            ...
            if ($tags) {
                $taglist = explode(",", str_replace("，", ",", $tags));
                $_ENV['tag']->multi_add($taglist, $qid);
            }
            //增加用户积分，扣除用户悬赏的财富
            if ($this->user['uid']) {
            ...
            if ($askfromuid) {
                ...
                $_ENV['message']->add($this->user['username'], $this->user['uid'], $touser['uid'], '问题求助:' . $title, $description . '<br /> <a href="' . SITE_URL . $this->setting['seo_prefix'] . 'question/view/' . $qid . $this->setting['seo_suffix'] . '">点击查看问题</a>');
            }
    ...
    function onview() {
        ...
        $rownum = $_ENV['question']->search_title_num($word, $qstatus); //获取总的记录数F
        /* 问题过期处理 */
    ...
    function oneditanswer() {
        ...
        if (0 == $status) {
            $this->message('修改回答成功！为了确保问答的质量，我们会对您的回答内容进行审核。请耐心等待......', 'BACK');
        } else {
    ...
    function onajaxtags() {
        if (isset($this->post['qtitle']) && trim($this->post['qtitle'])) {
        	header('Content-Type:text/html;charset=UTF-8');
            exit($_ENV['question']->get_tags(trim($this->post['qtitle']),','));
        }
        exit('');
    }
    ...
    function onedittitle() {
        ...
        $viewurl = urlmap('question/view/' . $qid, 2);
        if ('' == $title) {
            $this->message('问题标题不能为空!', $viewurl);
        } else {
            $_ENV['question']->renametitle($qid, $title);
    ...
    function oneditcont() {
        ...
        $qid = $this->post['qid'];
        $viewurl = urlmap('question/view/' . $qid, 2);
        ('' == $content ) && $this->message('问题内容不能为空!', $viewurl);
        $_ENV['question']->update_content(intval($this->post['qid']), $content);
        $this->message('问题内容修改成功!', $viewurl);
    }

...
class questionmodel {
	...
	function rownum_by_cfield_cvalue_status($cfield='cid1', $cvalue=0, $status=0,$ask_area='') {
        $condition = " 1=1 ";
        ($cfield && $cvalue != 'all') && $condition.=" AND $cfield=$cvalue ";
        isset($this->statustable[$status]) && $condition.=$this->statustable[$status];
		if ($ask_area!='' && intval($ask_area)>=0) {
			$condition.= " AND ask_area=$ask_area ";
		}
        return $this->db->fetch_total('question', $condition);
    }
	...
	function list_by_cfield_cvalue_status($cfield='cid1', $cvalue=0, $status=0, $start=0, $limit=10,$ask_area='') {

        $questionlist = array();
        $sql = "SELECT * FROM " . DB_TABLEPRE . "question WHERE 1=1 ";
        ($cfield && $cvalue != 'all') && ($sql.=" AND $cfield=$cvalue ");
		if ($ask_area!='' && intval($ask_area)>=0) {
			$sql .= " AND ask_area=$ask_area ";
		}
	...
	function isoklookquestion($qid,$user_id){
		//echo "SELECT buyjilu_id FROM ".DB_TABLEPRE."buyjilu WHERE question_id=$qid and uid=$user_id";
	...
	function countSellRenShuo($qid){
		$arr = $this->db->fetch_first("SELECT count(*) as t FROM ".DB_TABLEPRE."buyjilu WHERE question_id=$qid ");
		return $arr['t'];
	}
	/**
	 * 自己是否回复
	 ...
	 * @return boolean
	 */
	function isReQuestion($qid,$username){
		//echo "SELECT count(*)as t FROM ".DB_TABLEPRE."answer WHERE qid=$qid and author=$username";
		$sql = "SELECT count(*)as t FROM ".DB_TABLEPRE."answer WHERE qid=$qid and author='$username'";
		//echo $sql;
	...
	function add($title, $description, $hidanswer, $price, $cid, $cid1=0, $cid2=0, $cid3=0, $status=0,$ask_area=1) {
        ...
        $this->db->query("INSERT INTO " . DB_TABLEPRE . "question SET ask_area='$ask_area',cid='$cid',cid1='$cid1',cid2='$cid2',cid3='$cid3',authorid='$uid',author='$username',title='$title',description='$description',price='$price',time='{$this->base->time}',endtime='$endtime',hidden='$hidanswer',status='$status',ip='{$this->base->ip}',search_words='$search_words'");
	...
	$this->db->query("UPDATE `" . DB_TABLEPRE . "question` set status=4 WHERE status=1 and  price>0 AND `id` in ($qids)");
	...
	if ($ask_area!='') {
		$condition.=" AND ask_area=$ask_area AND title like '%$title%'";
	}
    ...
    //根据标题搜索问题
	function search_title($title, $status='1,2,6', $addbestanswer=0, $start=0, $limit=20,$ask_area='') {
        $questionlist = array();
        $search_words = $this->get_words($title);
		if ($ask_area!='') {
			$sql = $search_words ? "SELECT * FROM " . DB_TABLEPRE . "question WHERE MATCH(search_words) AGAINST('$search_words') AND ask_area=$ask_area AND status IN($status)":"SELECT * FROM " . DB_TABLEPRE . "question WHERE STATUS IN ($status) AND ask_area=$ask_area AND title LIKE '%$title%' ";
		}else {

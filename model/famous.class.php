...
class famousmodel {
    ...
    function get_list($answersize=3, $start=0, $limit=3) {
        ...
        $famouslist = array();
        $query = $this->db->query("SELECT u.uid,u.username,u.truename,u.questions,u.answers,u.signature,u.credit1,u.credit2,u.credit3,f.id,f.reason,f.time,q.title,q.id as qid FROM " . DB_TABLEPRE . "famous as f," . DB_TABLEPRE . "user as u LEFT JOIN " . DB_TABLEPRE . "question as q ON u.questions=q.id WHERE f.uid=u.uid ORDER BY f.id DESC LIMIT $start ,$limit",'SILENT');
        ...
        while ($famous = $this->db->fetch_array($query)) {
            ...
            $famous['truename'] = !empty($famous['truename'])?$famous['truename']:$famous['username'];
            $famouslist[] = $famous;
        }
        return $famouslist;
    }
